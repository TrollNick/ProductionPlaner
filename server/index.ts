  ...
  const create = db.transaction(() => {
    ...
    if (source_id) {
      ...
      for (const source of sourceItems) {
        const inserted = db.prepare(`INSERT INTO items (project_id, type, title, partner, start_date, end_date, status, notes, sort_order)
          VALUES (?, ?, ?, ?, ?, ?, 'open', ?, ?)`).run(id, source.type, source.title, source.partner, source.start_date, source.end_date, source.notes, source.sort_order);
...
app.post('/api/projects/:id/items', (req, res) => {
  const { type, title, partner = '', start_date, end_date, status = 'open', notes = '', dependency_ids = [] } = req.body;
  ...
  const create = db.transaction(() => {
    ...
    const result = db.prepare(`INSERT INTO items (project_id, type, title, partner, start_date, end_date, status, notes, sort_order)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(req.params.id, type, title, partner, start_date, end_date, status, notes, max.value + 1);
    const id = Number(result.lastInsertRowid);
...
app.patch('/api/items/:id', (req, res) => {
  ...
  const update = db.transaction(() => {
    db.prepare(`UPDATE items SET type = ?, title = ?, partner = ?, start_date = ?, end_date = ?, status = ?, notes = ?, sort_order = ? WHERE id = ?`)
      .run(next.type, next.title, next.partner, next.start_date, next.end_date, next.status, next.notes, next.sort_order, req.params.id);
